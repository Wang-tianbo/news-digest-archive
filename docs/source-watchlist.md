...
这部分用于长期跟踪中文互联网与全球范围内活跃且核心的 AI 博主、技术工作者、研究者、创业者及关键人物，在博客、公众号、Substack、GitHub、公开长文中表达的可复查线索、判断、困惑、分歧与方法论。

使用原则：

- 这是观点 / 线索源，不是事实源；不能用个人观点或资讯整理替代官方发布、文档、Release、changelog 或原始项目链接
- `AI 圈博主` 的入选门槛从“必须是强观点”调整为“可复查、AI 相关、对理解社区体感或工具演进有信息增量”；资讯整理、工具发现、社区争论、项目动态、方法论笔记都可以入选，但必须标明类型
- 核心日常雷达里的资讯整理型博主可以提供 AI 线索、工具发现、社区信号或产品动态，进入日报时必须标明 `类型：信息线索`
...
- 主窗口仍是前一日 09:00 到当日 09:00；如果主窗口命中不足，允许补入最近 7 天内未写入过、仍有复盘价值的公开观点 / 社区信号，并在条目中标明发布日期或观察日期
- 日报正文目标是 2-3 条 `AI 圈博主` 信号，最多 4 条；如果扩展巡检后仍只有 0-1 条，允许短，但必须在 `source_checks` 说明哪些来源组是 `miss`、`partial` 或 `blocked`
- 避免单一来源垄断：同一来源连续两天入选后，第三天只有在出现明确新观点、新方法或重大线索时才继续写入；不能把 Simon Willison 或任何单一稳定博客当作默认占位
...
- 中英文扩展源每天合计建议选 2-3 条有信息增量的社区 / 观点信号，最多 4 条；不要求每条都是强观点，但要避免重复前几天已经写过的内容
- 每天至少巡检中文公开源、英文个人 / newsletter 源、工程社区源三个桶；优先让正文里同时出现中文侧和英文侧样本，除非确实没有可复查新增
...
- 对 AI 圈博主，优先扫描核心日常雷达和扩展源池；资讯整理型博主、工程社区和媒体线索可以提供 `信息线索`，不要把“必须有强观点”当作入选前提
- 如果某天 `AI 圈博主` 只有 0-1 条，先扩大到最近 7 天未记录过的公开输出；仍不足时再省略或保留 1 条，并在 `source_checks` 说明原因
...
- 日报里的观点观察建议控制在 2-3 条、最多 4 条；如果扩展巡检后没有可复查信息增量，可以整段省略
