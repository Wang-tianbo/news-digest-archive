...
- 日报正文要求固定产出 5 条 `AI 圈博主` 信号：中文来源 2 条、英文来源 3 条；如果扩展巡检后仍无法凑齐，不能编造或重复旧内容，必须在 `source_checks` 说明哪些来源组是 `miss`、`partial` 或 `blocked`
...
- 中英文扩展源每天合计要求选 5 条有信息增量的社区 / 观点信号，其中中文来源 2 条、英文来源 3 条；不要求每条都是强观点，但要避免重复前几天已经写过的内容
...
- 如果某天 `AI 圈博主` 严格窗口不足 5 条，先扩大到最近 7 天未记录过的公开输出；仍不足时再保留已验证条目，并在 `source_checks` 说明缺口来源、访问状态和未凑齐原因
...
- 日报里的观点观察要求 5 条：中文来源 2 条、英文来源 3 条；如果扩展巡检后没有足够可复查信息增量，可以少于 5 条，但必须说明缺口
