...
> 本节每天要求 5 条：中文来源 2 条、英文来源 3 条。
> 如果严格窗口命中不足 5 条，可补入最近 7 天未写入过的公开输出，并标明发布日期或观察日期。
> 如果扩展巡检后仍不足 5 条，保留已验证条目，不编造、不重复旧内容，并在 `结构化索引` 的 `source_checks` 中说明缺口。
...
> 如果当天没有高价值论文或研究动向，省略本节，并在 `结构化索引` 中写 `research_sources: []`、`research_themes: []`、`research_artifacts: []` 和 `research_interpretations: []`。
